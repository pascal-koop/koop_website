...
<template>
    ...
    <h1  :style="{transform: `translateY(${-scrollPosition}px)`}" class="text-textLight text-4xl font-neueRegrade font-medium pt-6 inline-block mr-60">Pascal Koop</h1>
    <p :style="{transform: `translateY(${-scrollPosition}px)`}" class="inline-block text-textLight text-3xl font-neueRegrade font-medium">2024</p>
		<div class="flex items-center justify-end pt-80">
			<div class="absolute mt-6 flex flex-col introduction-content gap-7">
				...
				<h1
					:style="{ transform: `translateX(${-scrollPosition}px)` }"
					class="z-10 text-7xl font-neueRegrade font-medium title text-textLight self-end">
					DEVELOPER
				</h1>
				<button
					:style="{ transform: `translateX(${scrollPosition}px)` }"
					class="self-end font-rota font-normal text-xl tracking-[0.2em] border-[1px] pb-2 pt-3 px-7 rounded-full text-textLight z-20">
					JUNIOR
				</button>

				<NuxtImg
					class="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-[54%] z-1"
